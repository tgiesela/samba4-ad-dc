...
DIRS="${FROMWHERE}/private ${FROMWHERE}"
...
#	relativedirname=`find . -type d -name "$d" -prune`
	n=`echo $d | sed 's/\//_/g'`
	if [ "$d" = "${FROMWHERE}/private" ]; then
		find . -name "*.ldb.bak" -exec rm {} \;
		for ldb in `find . -name "*.ldb"`; do
			tdbbackup $ldb
		...
		tar cjf ${WHERE}/${n}.${WHEN}.tar.bz2  $d --exclude=\*.ldb --warning=no-file-ignored --transform 's/.ldb.bak$/.ldb/'
		Status=$?	# Preserve $? for message, since [ alters it.
